...
void pin_init(void)
{
    ...
    /* AFRL reset state */
    GPIO_AFRL(GPIOA) =
	    (GPIO_AFRL(GPIOA) & ~0x0000ff00) | (GPIO_AF4 << (2 * 4)) | (GPIO_AF4 << (3 * 4)) | (GPIO_AF5 << (5 * 4));
    /* AFRH reset state */
    ...
    GPIO_OSPEEDR(GPIOB) =
        ...
        (GPIO_OSPEED_HIGH << (5 * 2)) | (GPIO_OSPEED_HIGH << (6 * 2)) |
    ...
    /* af, alternative function selection */
    GPIO_AFRL(GPIOB) =
        (GPIO_AFRL(GPIOB) & ~0xff000000) | (GPIO_AF4 << (5 * 4)) | (GPIO_AF1 << (6 * 4)) |
        (GPIO_AF1 << (7 * 4));
    GPIO_AFRH(GPIOB) =
    ...
    GPIO_MODER(GPIOB) =
        ...
        (GPIO_MODE_AF << (5 * 2)) | (GPIO_MODE_AF << (6 * 2)) |
